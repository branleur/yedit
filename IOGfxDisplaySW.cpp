...
bool IOGfxDisplaySW::createRenderTexture(int w, int h) {
	...
	render_texture_nearest =
			SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB888,
							SDL_TEXTUREACCESS_STREAMING, 1280, 990);
...
void IOGfxDisplaySW::flip(IOGfxSurface* backbuffer, SDL_Rect* dstrect,
	...
	if (editor_mode > 0)
		dbg_imgui();
	//Yeolde: Stuck this here so I wouldn't ahve to make the texture unprivate
	if (debug_gamewindow) {
		...
		edit_edscale = 2;
		//if (debug_drawgame)
		ImGui::Image(render_texture_linear,ImVec2((int)640*edit_edscale,480*edit_edscale));
		//else {
		//ImGui::Image(render_texture_nearest,ImVec2((int)640, 480));
		//}
		ImGui::End();
	}
	/* Convert to destination buffer format */
	SDL_Surface* source = dynamic_cast<IOGfxSurfaceSW*>(backbuffer)->image; 
	SDL_Surface* sneed = dynamic_cast<IOGfxSurfaceSW*>(IOGFX_backbuffer)->image;
	...
	//change the last arg in rendercopy back to dstrect to make it stay within aspect ratio
	//what we want to do is make it so that the map boundaries in the background only trigger when the popout is open
	//otherwise, update both textures, as imgui has its own renderer that can display either
	if (interpolation) {
		SDL_UpdateTexture(render_texture_linear, NULL, source->pixels,
						source->pitch);
		if (debug_drawgame)
		SDL_RenderCopy(renderer, render_texture_linear, NULL, dstrect);
		
	} 
	//This holds the bounds preview
	if (!debug_drawgame) {
		SDL_UpdateTexture(render_texture_nearest, NULL, source->pixels,
						source->pitch);
						//sneed holds the main normal screen
		SDL_UpdateTexture(render_texture_linear, NULL, sneed->pixels, sneed->pitch);
		SDL_Rect r = {0, 0, this->w, this->h};
		SDL_RenderCopy(renderer, render_texture_nearest, &r, &r);
	} 
...
void IOGfxDisplaySW::onSizeChange(int w, int h) {
	//yeolde: for the background preview. Delete and recreate the texture upon screen resize to fit dimensions
	//I hope this doesn't leak (much)
	SDL_DestroyTexture(render_texture_nearest);
	render_texture_nearest = NULL;
	this->w = w;
	this->h = h;
	render_texture_nearest = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, w, h);
	SDL_RenderSetLogicalSize(renderer, w, h);
}
