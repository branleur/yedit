...
void fill_whole_hard(void) {
	...
	for (til = 0; til < 96; til++) {
		...
		for (x = 0; x < 50; x++)
			for (y = 0; y < 50; y++)
				screen_hitmap[offx + x][offy + y] =
						hmap.htile[realhard(til)].hm[x][y];
	}
}
...
/*bool*/ int get_box(int h, rect* box_scaled, rect* box_real,
	...
	if (getpic(h) < 1) {
		log_warn("Yo, sprite %d has a bad pic. Seq %d, Frame %d", h,
				spr[h].pseq, spr[h].pframe);
		goto nodraw;
	}
...
released, so we keep it for compatibility */
	...
	if (spr[h].alt.right != 0 || spr[h].alt.left != 0 || spr[h].alt.top != 0) {
		...
		box_scaled->right =
				box_scaled->right - (k[getpic(h)].box.right - spr[h].alt.right);
		box_scaled->bottom = box_scaled->bottom -
				(k[getpic(h)].box.bottom - spr[h].alt.bottom);
...
void draw_sprite_game(IOGfxSurface* GFX_lpdest, int h) {
	...
	it.. */
		...
		if (retval < 0) {
			log_error("Could not draw sprite %d: %s", getpic(h),
					SDL_GetError());
