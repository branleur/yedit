...
int IOGfxSurfaceSW::blit(IOGfxSurface* src, const SDL_Rect* srcrect,
						SDL_Rect* dstrect) {
	if (src == NULL)
...
int gfx_blit_stretch(SDL_Surface* src_surf, const SDL_Rect* src_rect_opt,
	...
	non-transparent surfaces) */
		Uint8 r, g, b, a;
		Uint32 colorkey;
		bool colorkey_enabled = (SDL_GetColorKey(src_surf, &colorkey) != -1);
		SDL_GetRGBA(colorkey, src_surf->format, &r, &g, &b, &a);
		
		Uint32 pix = SDL_MapRGBA(scaled->format, r, g, b, a);
		SDL_SetColorKey(scaled, colorkey_enabled, pix);
...
int IOGfxSurfaceSW::blitStretch(IOGfxSurface* src, const SDL_Rect* srcrect,
								SDL_Rect* dstrect) {
	if (src == NULL)
		return SDL_SetError(
...
int IOGfxSurfaceSW::blitNoColorKey(IOGfxSurface* src, const SDL_Rect* srcrect,
								SDL_Rect* dstrect) {
	if (src == NULL)
		return SDL_SetError(
