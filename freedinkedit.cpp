...
//yeolde: muh stuff since the above is declared static
int editor_mode;
int cur_sprite;
//For the benchmark
int startTime, render_time;
//screen displayed time for savestates
Uint32 screenTime;
...
static bool skip_screen_clipping() {
	return (g_editorMode == EDITOR_MODE_MINIMAP ||
...
void place_sprites() {
	...
	for (; r1 < MAX_SPRITES_EDITOR && rank[r1] > 0; r1++) {
		...
		if (cur_ed_screen.sprite[j].active == 1 &&
			(cur_ed_screen.sprite[j].vision == 0 ||
			(cur_ed_screen.sprite[j].vision == map_vision || edit_drawvis))) {
			//we have instructions to make a sprite

			if (cur_ed_screen.sprite[j].type == 0 ||
				cur_ed_screen.sprite[j].type == 2) {
				//make it part of the background (much faster)
				int sprite = add_sprite_dumb(cur_ed_screen.sprite[j].x,
											cur_ed_screen.sprite[j].y, 0,
											cur_ed_screen.sprite[j].seq,
											cur_ed_screen.sprite[j].frame,
			...
			//view invisible sprites with a toggle
			if ((cur_ed_screen.sprite[j].type == 1 && cur_ed_screen.sprite[j].seq != edit_forbiddenseq) || (cur_ed_screen.sprite[j].type == 2 && edit_viewinvis) ) {
...
//Loads a MAP.dat screen as per its index supplied in dink.dat
void editor_load_screen(int num) {
	//Bold assumption that they've just opened the thing and haven't done anything yet
	if (SDL_GetTicks() < 20000) {
		//This does not work on accursed maps that have used MapNuke
	memcpy(&cur_ed_screen, &ramscreens[num], sizeof(cur_ed_screen));
	log_debug("Rendering screen %d from RAM", num);
	}
	else
	if (load_screen_to(g_dmod.map.map_dat.c_str(), num, &cur_ed_screen) < 0)
		return;
	
	if (edit_fillhard && draw_screen_tiny == -1)
	fill_whole_hard();
	//Don't render sprite previews while minimapping
	if (draw_screen_tiny == -1)
	render_sprite_preview();
	//Reset our sprite number upon screen load
	spri = 1;
}
...
void draw_screen_editor(void) {
	...
	if (edit_drawtiles) {
		IOGFX_background->fill_screen(tile_hide_col, GFX_ref_pal);
	}
	else
...
void draw_current() {
	...
	IOGFX_background->blit(g_dmod.bgTilesets.slots[srctileset_idx0 + 1], &src,
						&dst);
}
		...
		for (y = 0; y < 50; y++) {
			...
			if (hmap.htile[hard_tile].hm[x][y] == 1) {
				...
				// GFX
				{
					...
					dst.x = 95 + x * 9;
					dst.y = y * 9;
					IOGFX_backbuffer->blit(GFX_k[seq[10].frame[2]].k, NULL,
										&dst);
				}
			}
			...
			if (hmap.htile[hard_tile].hm[x][y] == 2) {
				...
				// GFX
				{
					...
					dst.x = 95 + x * 9;
					dst.y = y * 9;
					IOGFX_backbuffer->blit(GFX_k[seq[10].frame[9]].k, NULL,
										&dst);
				}
			}
			...
			if (hmap.htile[hard_tile].hm[x][y] == 3) {
				...
				// GFX
				{
					...
					dst.x = 95 + x * 9;
					dst.y = y * 9;
					IOGFX_backbuffer->blit(GFX_k[seq[10].frame[10]].k, NULL,
										&dst);
				}
			}
		}
...
void draw_this_map(EditorMap* pmap) {
	int x;
	for (x = 0; x < 768; x++) {
		...
		if (pmap->loc[x + 1] == 0) {
			...
			// GFX
			{
				...
				dst.x = x * 20 - x / 32 * 640;
				dst.y = x / 32 * 20;
				IOGFX_background->blitNoColorKey(GFX_k[seq[10].frame[6]].k,
												NULL, &dst);
			}
		}
		...
		if (pmap->loc[x + 1] > 0) {
			...
			// GFX
			{
				...
				dst.x = x * 20 - x / 32 * 640;
				dst.y = x / 32 * 20;
				IOGFX_background->blitNoColorKey(GFX_k[seq[10].frame[7]].k,
												NULL, &dst);
			}
		}
...
void check_joystick() {
	...
	if (input_getscancodestate(SDL_SCANCODE_ESCAPE))
		sjoy.joybit[EDITOR_ACTION_ESCAPE] = 1;
	if (input_getscancodestate(SDL_SCANCODE_RETURN))
		sjoy.joybit[EDITOR_ACTION_RETURN] = 1;
	if (input_getcharstate(SDLK_x))
		sjoy.joybit[EDITOR_ACTION_X] = 1;
	if (input_getcharstate(SDLK_z))
		sjoy.joybit[EDITOR_ACTION_Z] = 1;
	if (input_getscancodestate(SDL_SCANCODE_TAB))
		sjoy.joybit[EDITOR_ACTION_TAB] = 1;
...
void loadtile(int tileset) {
	...
	if (g_editorMode == EDITOR_MODE_SCREEN_TILES) {
		...
		spr[1].x = m2x;
		spr[1].y = m2y;
		g_editorMode = EDITOR_MODE_TILE_PICKER;
		spr[1].speed = 50;
	}
		...
		for (y1 = 0; y1 <= 7; y1++) {
			...
			if (se > 0) {
				...
				// GFX
				{
					...
					dst.w = 50;
					dst.h = 50;
					IOGFX_background->blitStretch(GFX_k[seq[se].frame[frame]].k,
												NULL, &dst);
				}
	...
	for (x1 = 0; x1 <= 11; x1++) {
		...
		for (y1 = 0; y1 <= 7; y1++) {
			...
			// GFX
			{
				...
				dst.w = 50;
				dst.h = 50;
				IOGFX_background->blitStretch(GFX_k[seq[se].frame[num]].k, NULL,
											&dst);
			}
...
void sp_add(void) {
	...
	for (j = 1; j < 100; j++) {
		if (cur_ed_screen.sprite[j].active <= /*false*/ 0) {
			...
			last_sprite_added = j;
			log_debug("Adding sprite %d, seq %d, frame %d.",j,sp_seq,sp_frame);
			...
			if (edit_nohit)
			cur_ed_screen.sprite[j].nohit = 1;
			else
		...
		//else
		//log_debug("Could not allocate sprite! %d", j);
	}
}
...
void smart_add(void) {
	...
	if (sp_screenmatch) {
		...
		if (spr[1].size == 100) {
			...
			if ((spr[1].x - k[getpic(1)].xoffset) + k[getpic(1)].box.right >
				620) {
			...
			if ((spr[1].y - k[getpic(1)].yoffset) + k[getpic(1)].box.bottom >
				400) {
...
**/
	...
	if (in_master == 2) {
		...
		Say("Type controls the sprite's basic type - 0 means it is ornamental "
			"only"
			"(cannot walk behind or think) 1 - means normal sprite.  (for a "
			"tree or person)",
			10, 10);
	}
	...
	if (in_master == 3) {
		...
		Say("Brains are a predefined way for this sprite to behave. 0 = No "
			"movement, 1 = Dink,"
			" 2 = Dumb Sprite Bouncer, 3 = Duck, 4 = Pig, 6 = repeat, 7 = one "
			"loop then kill,"
	...
	if (in_master == 4) {
		...
		Say("Speed rating allows you to adjust how fast a certain sprite "
			"moves.  Works with"
			" most brains.",
			10, 10);
	}
	...
	if (in_master == 5) {
		...
		Say("This is the delay the CPU waits before processing the sprite "
			"after each cycle.  "
	...
	if (in_master == 6) {
		...
		Say("The base for which the CPU adds 1 through 9 to make the sprite "
			"move, depending on"
			" direction.  Must be a multiple of ten. (ie, 20 to look like a "
			"duck, 40 to look like a pig)",
			10, 10);
	}
	...
	if (in_master == 7) {
		...
		Say("Some brains can optionally use extra sprites for their \'idle\' "
			"pose.",
			10, 10);
	}
	...
	if (in_master == 8) {
		...
		Say("From 1 to 20000, 0 for default.  (defaults to y cord)", 10, 10);
	}
	...
	if (in_master == 9) {
		...
		Say("Sets how hardness works.  1 means normal, (monsters) 0 means "
			"added to background. (walls, trees)",
			10, 10);
	}
	...
	if (in_master == 10) {
		...
		Say("Sets special properties for the hardblock.  0 = normal (just "
			"hard) 1 = warp.",
			10, 10);
	}
	...
	if (in_master == 11) {
		...
		Say("These parms are valid if the hard block property setting is 1.  "
			"(warp)",
			10, 10);
	}
	...
	if (in_master == 12) {
		...
		blit(30, 1, IOGFX_backbuffer, 250, 170);
		Say("Warp X:", 260, 175);
		Say("The X location to warp to.  (20 to 619)", 10, 10);
	}
	...
	if (in_master == 13) {
		...
		blit(30, 1, IOGFX_backbuffer, 250, 170);
		Say("Warp Y:", 260, 175);
		Say("The Y location to warp to.  (0 to 499)", 10, 10);
	}
	...
	if (in_master == 14) {
		...
		Say("This parm is used by some brains/settings if set.  A sequence is "
			"an animation #.",
			10, 10);
	}
	...
	if (in_master == 15) {
		...
		blit(30, 1, IOGFX_backbuffer, 250, 170);
		Say("Script:", 260, 175);
		Say("Filename of script this sprite uses.", 10, 10);
	}
	...
	if (in_master == 16) {
		...
		Say("If this sprite dies, this will be used.", 10, 10);
	}
	...
	if (in_master == 17) {
		...
		blit(30, 1, IOGFX_backbuffer, 250, 170);
		Say("Sound:", 260, 175);
		Say("This sprite will play this sound looped until it dies.", 10, 10);
	}
	...
	if (in_master == 18) {
		...
		Say("How strong is this creature?  (0 = not alive/invincible)", 10, 10);
	}
	...
	if (in_master == 21) {
		...
		Say("If not -1, this monster can attack with this sprite base. (base + "
			"dir)",
			10, 10);
	}
	...
	if (in_master == 22) {
		...
		blit(30, 1, IOGFX_backbuffer, 250, 170);
		Say("Defense:", 260, 175);
		Say("This will be deducted from any attack.", 10, 10);
	}
	...
	if (in_master == INPUT_SCREEN_MIDI) {
		...
		Say("Will play #.MID for this screen if nothing else is playing.", 10,
			10);
	}
...
void shrink_screen_to_these_cords(int x1, int y1) {
	...
	//log_debug("X value is %d", x1);
	{
		...
		dst.w = 20;
		dst.h = 20;
		IOGFX_tmp2->blitStretch(IOGFX_background, &src, &dst);
		//IOGFX_backbuffer->blitStretch(IOGFX_background, &src, &dst);
	} //yeolde: copies to the temp buffer so it may be seen separately
}
	...
	for (x = 0; x < 50; x++) {
		...
		for (y = 0; y < 50; y++) {
			if (hmap.htile[tile].hm[x][y] == 1) {
				//draw it
				dst.x = x1 + x + 20 /* status bar length */;
				dst.y = y1 + y;
...
int gui_logic(int h) {
		...
		if (g_editorMode == EDITOR_MODE_SPRITE_HARDNESS) {
			...
			if (input_getcharjustpressed(SDLK_s)) {
				...
				sprintf(death, "SET_SPRITE_INFO %d %d %d %d %d %d %d %d\n",
						sp_seq, sp_frame,
						k[seq[sp_seq].frame[sp_frame]].xoffset,
						k[seq[sp_seq].frame[sp_frame]].yoffset,

						k[seq[sp_seq].frame[sp_frame]].hardbox.left,
						k[seq[sp_seq].frame[sp_frame]].hardbox.top,
						k[seq[sp_seq].frame[sp_frame]].hardbox.right,
			...
			if (sp_mode == 2) {
				...
				if (k[seq[sp_seq].frame[sp_frame]].hardbox.right <=
					k[seq[sp_seq].frame[sp_frame]].hardbox.left)
					k[seq[sp_seq].frame[sp_frame]].hardbox.left =
							k[seq[sp_seq].frame[sp_frame]].hardbox.right - 1;

				if (k[seq[sp_seq].frame[sp_frame]].hardbox.bottom <=
					k[seq[sp_seq].frame[sp_frame]].hardbox.top)
					k[seq[sp_seq].frame[sp_frame]].hardbox.top =
							k[seq[sp_seq].frame[sp_frame]].hardbox.bottom - 1;
			}
			...
			if (sp_mode == 1) {
				...
				if (k[seq[sp_seq].frame[sp_frame]].hardbox.left >=
					k[seq[sp_seq].frame[sp_frame]].hardbox.right)
					k[seq[sp_seq].frame[sp_frame]].hardbox.right =
							k[seq[sp_seq].frame[sp_frame]].hardbox.left + 1;

				if (k[seq[sp_seq].frame[sp_frame]].hardbox.top >=
					k[seq[sp_seq].frame[sp_frame]].hardbox.bottom)
					k[seq[sp_seq].frame[sp_frame]].hardbox.bottom =
							k[seq[sp_seq].frame[sp_frame]].hardbox.bottom + 1;
			}

			if (k[seq[sp_seq].frame[sp_frame]].hardbox.top > 200)
				k[seq[sp_seq].frame[sp_frame]].hardbox.top = 200;
			if (k[seq[sp_seq].frame[sp_frame]].hardbox.top < -200)
				k[seq[sp_seq].frame[sp_frame]].hardbox.top = -200;

			if (k[seq[sp_seq].frame[sp_frame]].hardbox.left > 316)
				k[seq[sp_seq].frame[sp_frame]].hardbox.left = 316;
			if (k[seq[sp_seq].frame[sp_frame]].hardbox.left < -320)
				k[seq[sp_seq].frame[sp_frame]].hardbox.left = -320;

			if (k[seq[sp_seq].frame[sp_frame]].hardbox.bottom > 200)
				k[seq[sp_seq].frame[sp_frame]].hardbox.bottom = 200;
			if (k[seq[sp_seq].frame[sp_frame]].hardbox.bottom < -200)
				k[seq[sp_seq].frame[sp_frame]].hardbox.bottom = -200;

			if (k[seq[sp_seq].frame[sp_frame]].hardbox.right > 316)
				k[seq[sp_seq].frame[sp_frame]].hardbox.right = 316;
			if (k[seq[sp_seq].frame[sp_frame]].hardbox.right < -320)
				k[seq[sp_seq].frame[sp_frame]].hardbox.right = -320;
		...
		if (g_editorMode == EDITOR_MODE_SCREEN_SPRITES) {
			...
			*/
			if (!((spr[1].pseq == 10) && (spr[1].pframe == 8))) {
				...
				if (input_getscancodestate(SDL_SCANCODE_PAGEDOWN) ||
					input_getscancodestate(SDL_SCANCODE_LEFTBRACKET))
					spr[1].size -= 1 + modif;
				/* if (GetKeyboard(VK_OEM_6 /\* 221 *\/)) // ']' for US */
				if (input_getscancodestate(SDL_SCANCODE_PAGEUP) ||
					input_getscancodestate(SDL_SCANCODE_RIGHTBRACKET))
					spr[1].size += 1 + modif;

				if (SDL_GetModState() & KMOD_SHIFT) {
					//shift is being held down
					if (input_getscancodestate(SDL_SCANCODE_1) ||
						input_getscancodestate(SDL_SCANCODE_KP_1) ||
						input_getscancodestate(SDL_SCANCODE_F1))
						in_master = 11;
					if (input_getscancodestate(SDL_SCANCODE_2) ||
						input_getscancodestate(SDL_SCANCODE_KP_2) ||
						input_getscancodestate(SDL_SCANCODE_F2))
						in_master = 12;
					if (input_getscancodestate(SDL_SCANCODE_3) ||
						input_getscancodestate(SDL_SCANCODE_KP_3) ||
						input_getscancodestate(SDL_SCANCODE_F3))
						in_master = 13;
					if (input_getscancodestate(SDL_SCANCODE_4) ||
						input_getscancodestate(SDL_SCANCODE_KP_4) ||
						input_getscancodestate(SDL_SCANCODE_F4))
						in_master = 14;
					if (input_getscancodestate(SDL_SCANCODE_5) ||
						input_getscancodestate(SDL_SCANCODE_KP_5) ||
						input_getscancodestate(SDL_SCANCODE_F5))
						in_master = 15;

					if (input_getscancodestate(SDL_SCANCODE_6) ||
						input_getscancodestate(SDL_SCANCODE_KP_6) ||
						input_getscancodestate(SDL_SCANCODE_F6))
						in_master = 16;
					if (input_getscancodestate(SDL_SCANCODE_7) ||
						input_getscancodestate(SDL_SCANCODE_KP_7) ||
						input_getscancodestate(SDL_SCANCODE_F7))
						in_master = 17;
					if (input_getscancodestate(SDL_SCANCODE_8) ||
						input_getscancodestate(SDL_SCANCODE_KP_8) ||
						input_getscancodestate(SDL_SCANCODE_F8))
						in_master = 18;
					if (input_getscancodestate(SDL_SCANCODE_9) ||
						input_getscancodestate(SDL_SCANCODE_KP_9) ||
						input_getscancodestate(SDL_SCANCODE_F9))
						in_master = 19;
					...
					*/
					...
					if (input_getscancodestate(SDL_SCANCODE_9) ||
						...
						input_getscancodestate(SDL_SCANCODE_F9))
						in_master = 9;
					if (input_getscancodestate(SDL_SCANCODE_0) ||
						input_getscancodestate(SDL_SCANCODE_KP_0) ||
						input_getscancodestate(SDL_SCANCODE_F10))
						in_master = 10;
				}
				...
				for (jj = 1; jj < 100; jj++) {
					if (cur_ed_screen.sprite[jj].active)
						if (cur_ed_screen.sprite[jj].vision == map_vision)
							max_spr++;
				}
				...
				if (max_spr > 0) {
					...
					if (input_getscancodejustpressed(SDL_SCANCODE_PAGEUP) ||
						input_getscancodejustpressed(
					...
					if (input_getscancodejustpressed(SDL_SCANCODE_PAGEDOWN) ||
						input_getscancodejustpressed(
				...
				if (sp_cycle > 0) {
					...
					for (jh = 1; dumbpic != sp_cycle; jh++) {
						if (cur_ed_screen.sprite[jh].active)
							if (cur_ed_screen.sprite[jh].vision == map_vision) {
								dumbpic++;
								realpic = jh;
							}
						if (jh == 99)
			...
			fail:
				if ((sjoy.button[EDITOR_ACTION_RETURN]) | (mouse1)) {
					...
					for (uu = 100; uu > 0; uu--) {
						if (cur_ed_screen.sprite[uu].active)
							if ((cur_ed_screen.sprite[uu].vision == 0) ||
								(cur_ed_screen.sprite[uu].vision ==
								map_vision)) {

								int sprite = add_sprite_dumb(
										cur_ed_screen.sprite[uu].x,
										cur_ed_screen.sprite[uu].y, 0,
										cur_ed_screen.sprite[uu].seq,
										cur_ed_screen.sprite[uu].frame,
										cur_ed_screen.sprite[uu].size);
								rect_copy(&spr[sprite].alt,
										&cur_ed_screen.sprite[uu].alt);
								get_box(sprite, &box_crap, &box_real,
										skip_screen_clipping());
								if (realpic > 0)
									goto spwarp;
								...
								if (inside_box(spr[1].x, spr[1].y, box_crap)) {
									...
									sp_base_walk =
											cur_ed_screen.sprite[uu].base_walk;
									sp_base_idle =
											cur_ed_screen.sprite[uu].base_idle;
									sp_base_attack = cur_ed_screen.sprite[uu]
															.base_attack;
									sp_base_hit =
											cur_ed_screen.sprite[uu].base_hit;
									...
									sp_is_warp =
											cur_ed_screen.sprite[uu].is_warp;

									sp_warp_map =
											cur_ed_screen.sprite[uu].warp_map;
									sp_warp_x = cur_ed_screen.sprite[uu].warp_x;
									sp_warp_y = cur_ed_screen.sprite[uu].warp_y;
									sp_parm_seq =
											cur_ed_screen.sprite[uu].parm_seq;
									strcpy(sp_script,
										cur_ed_screen.sprite[uu].script);

									sp_base_die =
											cur_ed_screen.sprite[uu].base_die;
									sp_gold = cur_ed_screen.sprite[uu].gold;
									sp_hitpoints =
											cur_ed_screen.sprite[uu].hitpoints;

									sp_exp = cur_ed_screen.sprite[uu].exp;
									sp_nohit = cur_ed_screen.sprite[uu].nohit;
									sp_touch_damage = cur_ed_screen.sprite[uu]
															.touch_damage;
									sp_defense =
											cur_ed_screen.sprite[uu].defense;
									sp_strength =
											cur_ed_screen.sprite[uu].strength;
									sp_sound = cur_ed_screen.sprite[uu].sound;

									cur_ed_screen.sprite[uu].active =
											/*false*/ 0; //erase sprite
									draw_screen_editor();
									lsm_remove_sprite(sprite);
									break;
								}
				...
				if ((SDL_GetModState() & KMOD_ALT) &&
					(input_getscancodestate(SDL_SCANCODE_DELETE))) {
					int ll;
					for (ll = 1; ll < 100; ll++) {
		...
		if ((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			...
			if (sp_seq != 0) {
				...
				if (input_getcharjustpressed(SDLK_e)) {
					...
					holdx = holdx * 8;
					if (seq[sp_seq].frame[holdx + holdy] == 0)
						goto sp_fin;
			...
			if (sjoy.button[EDITOR_ACTION_TAB] ||
				sjoy.button[EDITOR_ACTION_ESCAPE]) {
			...
			/* if (sjoy.keyjustpressed[VK_OEM_4 /\* 219 *\/]) // '[' for US */
			if (input_getscancodejustpressed(SDL_SCANCODE_PAGEUP) ||
				input_getscancodejustpressed(SDL_SCANCODE_LEFTBRACKET)) {
				if (sp_picker > 95)
					sp_picker -= 96;
				else {
					sp_picker = (4 * 96);
			...
			if (input_getscancodejustpressed(SDL_SCANCODE_PAGEDOWN) ||
				input_getscancodejustpressed(SDL_SCANCODE_RIGHTBRACKET)) {
				if (sp_picker < 400)
					sp_picker += 96;
				draw15(sp_picker);
			}
	...
	sp_edit_end:
		...
		if (g_editorMode == EDITOR_MODE_INIT) {
			//yeolde: load screens into RAM upon starting
			for (int i=1; i<769; i++) {
		...
		if (g_editorMode == EDITOR_MODE_TILE_HARDNESS) {
			if (spr[h].seq == 0) {
				if ((SDL_GetModState() & KMOD_SHIFT) &&
					(input_getscancodestate(SDL_SCANCODE_RIGHT))) {
					spr[h].seq = 4;
					spr[h].frame = 1;
					if (selx < 8)
						selx++;
				...
				if ((SDL_GetModState() & KMOD_SHIFT) &&
					(input_getscancodestate(SDL_SCANCODE_LEFT))) {
					spr[h].seq = 4;
					spr[h].frame = 1;
					if (selx > 1)
						selx--;
				...
				if ((SDL_GetModState() & KMOD_SHIFT) &&
					(input_getscancodestate(SDL_SCANCODE_UP))) {
					spr[h].seq = 4;
					spr[h].frame = 1;
					if (sely > 1)
						sely--;
				...
				if ((SDL_GetModState() & KMOD_SHIFT) &&
					(input_getscancodestate(SDL_SCANCODE_DOWN))) {
					spr[h].seq = 4;
					spr[h].frame = 1;
					if (sely < 8)
						sely++;
		...
		b1fun:
			...
			if (g_editorMode != 1) {
				...
				if (spr[h].x + (9 * (selx - 1)) > 95 + 441)
					spr[h].x = (95 + 441) - (9 * (selx - 1));
				if (spr[h].x < 95)
					spr[h].x = 95;
				if (spr[h].y < 0)
					spr[h].y = 0;
				if (spr[h].y + (9 * (sely - 1)) > 441)
					spr[h].y = 441 - (9 * (sely - 1));
			}
			...
			if (input_getcharstate(SDLK_z)) {
				...
				for (y = 0; y < sely; y++) {
					int x;
					for (x = 0; x < selx; x++) {
						hmap.htile[hard_tile].hm[((spr[h].x) + (x * 9) - 95) /
												9][(spr[h].y + (y * 9)) / 9] =
								1;
					}
				}
		...
		if (((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			(sjoy.button[EDITOR_ACTION_RETURN])) ||
			((g_editorMode == EDITOR_MODE_TILE_PICKER) &&
			(input_getscancodestate(SDL_SCANCODE_SPACE)))) {

			if (g_editorMode == EDITOR_MODE_SCREEN_TILES)
				cur_tile = cur_ed_screen
								.t[(((spr[1].y + 1) * 12) / 50) +
									(spr[1].x / 50)]
								.square_full_idx0;
			...
			tilesel:
				...
				before scaling it.. */
				{
					...
					cool = cur_tile / 128;
					IOGFX_background->blitStretch(
							g_dmod.bgTilesets.slots[cool + 1], &src, &dst);
				}
		...
		if ((input_getcharstate(SDLK_s)) &&
			...
			cur_ed_screen.t[(((spr[1].y + 1) * 12) / 50) + (spr[1].x / 50)]
					.square_full_idx0 = cur_tile;
			...
			for (y = 0; y < sely; y++) {
				...
				for (x = 0; x < selx; x++) {
					...
					holdx += (y * 12);
					holdx += x;
					cur_ed_screen.t[holdx].square_full_idx0 =
							(cur_tile + (y * 12) + x);
				}
			}
		...
		if (g_editorMode == EDITOR_MODE_SCREEN_TILES ||
			...
			if (input_getscancodestate(SDL_SCANCODE_BACKSPACE) || input_getscancodestate(SDL_SCANCODE_KP_BACKSPACE))
				unit = ts;
			...
			if ((SDL_GetModState() & KMOD_ALT) &&
				...
				loadtile(41);
			/* alternatives */
			if (input_getscancodestate(SDL_SCANCODE_F11) ||
				input_getscancodestate(SDL_SCANCODE_KP_PERIOD))
				loadtile(41);
		}
		...
		if ((input_getscancodejustpressed(SDL_SCANCODE_SPACE)) &&
			(g_editorMode == EDITOR_MODE_MINIMAP)) {
			//make_map_tiny();
			draw_screen_tiny = 0;
		...
		if ((input_getcharjustpressed(SDLK_l)) &&
			(g_editorMode == EDITOR_MODE_MINIMAP)) {
		...
		if ((input_getcharjustpressed(SDLK_m)) &&
			...
			in_int = &g_dmod.map.music[(((spr[1].y + 1) * 32) / 20) +
									(spr[1].x / 20)];
		...
		if ((input_getcharjustpressed(SDLK_s)) &&
			(g_editorMode == EDITOR_MODE_MINIMAP)) {
			//set music # for this block
			in_int = &g_dmod.map.indoor[(((spr[1].y + 1) * 32) / 20) +
										(spr[1].x / 20)];
			in_master = INPUT_SCREEN_TYPE;
		}

		if ((input_getcharjustpressed(SDLK_q)) &&
			(g_editorMode == EDITOR_MODE_MINIMAP)) {
			save_hard();
			log_info("Info saved.");
			SDL_Event ev;
		...
		if ((sjoy.button[EDITOR_ACTION_RETURN]) &&
			...
			if (buf_mode) {
				...
				if (!load_screen_buf(buffmap.loc[(((spr[1].y + 1) * 32) / 20) +
												(spr[1].x / 20)])) {
					draw_minimap();
				...
				if (g_dmod.map.loc[(((spr[1].y + 1) * 32) / 20) +
								(spr[1].x / 20)] == 0) {

					(g_dmod.map.loc[(((spr[1].y + 1) * 32) / 20) +
									(spr[1].x / 20)]) = add_new_map();
					//wrongo, let's add the map

					//draw_minimap();
				}

				g_dmod.map.indoor[buf_map] =
						buffmap.indoor[(((spr[1].y + 1) * 32) / 20) +
									(spr[1].x / 20)];
				g_dmod.map.music[buf_map] =
						buffmap.music[(((spr[1].y + 1) * 32) / 20) +
									(spr[1].x / 20)];
		...
		if ((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			(input_getscancodestate(SDL_SCANCODE_PAGEUP) ||
			input_getscancodestate(SDL_SCANCODE_MINUS))) {
			spr[h].seq = 3;
			spr[h].seq_orig = 3;
			cur_tile--;
			if (cur_tile < 0)
		...
		if ((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			(input_getscancodestate(SDL_SCANCODE_PAGEDOWN) ||
			input_getscancodestate(SDL_SCANCODE_EQUALS))) {
			spr[h].seq = 3;
			spr[h].seq_orig = 3;
		...
		if ((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			(input_getcharjustpressed(SDLK_h))) {
			//start althard mode
		...
		if (g_editorMode == EDITOR_MODE_SCREEN_HARDNESS) {
			...
			if (input_getscancodejustpressed(SDL_SCANCODE_RETURN)) {
				...
				cur_tile = cur_ed_screen.t[xy2screentile(spr[1].x, spr[1].y)]
								.square_full_idx0;
		...
		if ((g_editorMode == EDITOR_MODE_SCREEN_TILES) &&
			...
			//Add to the minimap if the user has chosen to while exiting the screen
			if (edit_minimapupdate) {
				// This gets rid of the white box - or maybe not
				spr[1].active = 0;
				copy_front_to_two();
				//shrink_screen_to_these_cords((scr - 1) * 20 -((((scr - 1) / 32) * 640)),((((scr - 1) / 32)) * 20));
				//copy_screen_big((scr - 1) * 600 - ((((scr - 1) / 32) * (600 * 32))), ((((scr - 1) / 32)) * 400));
				update_minimap_view();
				spr[1].active = 1;
			}
			//changed this to use our screen locator var
...
void AppFreeDinkedit::logic(void) {
	...
	tiny_again:
		if (draw_screen_tiny == 769) {
			draw_screen_tiny = -1;
			minimap_status = draw_screen_tiny;
			lsm_kill_all_nonlive_sprites();
			if (!edit_minimapbigrend) {
			//Don't refresh the minimap prev if we're just looking at screen boundaries
			update_minimap_view();
			}
			//render_minimap_big();
			//Benchmark timer! Beat your friends!
			int endTime = SDL_GetTicks();
			render_time = endTime - startTime;
			//all done
		} else {
			//le hack to make it render only the big map preview 
			if (draw_screen_tiny == (scr + 2) && edit_minimapbigrend)
			draw_screen_tiny = 768;
			if (draw_screen_tiny == 0 && edit_minimapbigrend && (scr - 33) > 0)
			draw_screen_tiny = scr - 33;
			if (edit_minimapbigrend && draw_screen_tiny == scr - 30 && (scr - 30) > 0)
			draw_screen_tiny = scr - 2;
			minimap_status++;
			draw_screen_tiny++;
			//yeolde: so I can get minimap status
			copy_front_to_two();
	...
	if (!in_enabled)
		...
		for (jj = 1; jj < max_s; jj++) {
			...
			if (spr[h].active) {
				...
				//Msg("Drew %d.",h);
			// To attempt to get rid of that damn white box while previewing
			skip_draw:
				if (spr[h].brain == 1 && draw_screen_tiny == -1) {
					if (g_editorMode == EDITOR_MODE_SCREEN_TILES ||
						g_editorMode == EDITOR_MODE_TILE_PICKER) {
						...
						for (y = 0; y < sely; y++) {
							...
							for (x = 0; x < selx; x++) {
								...
								// GFX
								{
									...
									dst.y = spr[h].y + 50 * y;
									IOGFX_backbuffer->blit(GFX_k[getpic(h)].k,
														NULL, &dst);
								}
							}
						}
...
*/
	...
	if (g_editorMode == EDITOR_MODE_MINIMAP) {
		//yeolde: screen indicator
		scr = (((spr[1].y + 1) * 32) / 20) + (spr[1].x / 20);

		if (20 * (spr[1].y / 20) != spr[1].y) {
			spr[1].y += 10;
		}
		sprintf(msg,
				"Mode # %d - Press ENTER to edit, SPACE to detail map. (%d)  "
				"(Q) to quit and save. L to replace a \n"
				"screen from another map file. Z to toggle this help text. S "
				"to modify screentype. M to modify midi.",
				g_editorMode, (((spr[1].y + 1) * 32) / 20) + (spr[1].x / 20));
		/* 		  g_dmod.map.loc[(((spr[1].y+1)*32) / 20)+(spr[1].x / 20) ]); */
	}
	if (g_editorMode == EDITOR_MODE_SCREEN_TILES) {
		//((x-1) - (x / 12))*50, (x / 12) * 50,
		sprintf(msg,
				"Mode # %d, (C)opy or (S)tamp tile. ESC to exit to map picker. "
				"ENTER to edit hardness.\n TAB for sprite edit mode. 1-10 for "
				"tilescreens. (hold alt, crtl or shift for more) \n SPACE to show "
				"hardness of screen. (I)nfo on sprites."
				" V to change vision, B to change maps base .C file, H to edit "
				"tile hardness.",
				g_editorMode);
		/* 		  cur_map, */
		/* 		  cur_tile, */
	...
	if (g_editorMode == EDITOR_MODE_TILE_PICKER)
		sprintf(msg,
				"Mode # %d - Screen # %d - ENTER to choose, SPACE to edit "
				"hardness.",
				g_editorMode, cur_map);
	/* 		(((spr[1].y+1)*12) / 50)+(spr[1].x / 50)); */
	if (g_editorMode == EDITOR_MODE_TILE_HARDNESS) {
		sprintf(msg,
				"X:%d Y:%d: Density index %d  Z to harden, X to soften, A for "
				"low-hard, S for ???.\n"
				"Alt+X/A/S to fill hardness. Shift+direction for larger brush. "
	...
	if (g_editorMode == EDITOR_MODE_SPRITE_PICKER) {
		...
		if (sp_seq == 0) {
			sprintf(msg,
					"Choose sequence and press ENTER.  PageDown for next page, "
					"PageUp for previous. ESC or TAB to exit. (now on page %d)",
					1 + (sp_picker / 96));
		} else {
			sprintf(msg,
					"Choose a sprite from sequence %d.  Enter to place sprite, "
					"TAB to exit or ESC to return to previous screen."
	...
	if (g_editorMode == EDITOR_MODE_SCREEN_SPRITES) {
		...
		char crap7[80];
		if (sp_screenmatch)
			strcpy(crap7, "ScreenMatch is ON.");
		else
			strcpy(crap7, "ScreenMatch is OFF");

		sprintf(msg,
				"Press ENTER to pickup/putdown sprite. Space to show hardness. "
				" E to edit/pick new sprite. SHIFT to move fast. (S)tamp "
				"sprite. PageUp&"
				"PageDown to scale (now at %d).\n DEL to erase sprite.  Press 1 "
				"through 9 to change sprite attributes. (hold alt or shift for "
				"more)  Last sprite touched: %d  %s (M to toggle)"
				"Hold Z or X + arrow keys to trim a sprite. V to change Vision "
				"mode. X: %d Y: %d",
	...
	if (g_editorMode == EDITOR_MODE_SPRITE_HARDNESS) {
		...
		if (sp_mode == 0) {
			sprintf(msg,
					"Editing depth dot for Seq %d, frame %d.  SHIFT to move "
					"fast. Control to move one"
					" pixel.  TAB for next edit option. ESC to return to "
					"sprite picker. S to save to dink.ini.",
					sp_seq, sp_frame);
		}
		...
		{
			sprintf(msg,
					"Editing hardbox up left cordinate for Seq %d, frame %d.  "
					"SHIFT to move fast. Control to move one"
					" pixel.  TAB for next edit option. ESC to return to "
					"sprite picker. S to save to dink.ini. X: %d Y: %d",
					sp_seq, sp_frame,
					k[seq[sp_seq].frame[sp_frame]].hardbox.left,
					k[seq[sp_seq].frame[sp_frame]].hardbox.top);
		}
		...
		{
			sprintf(msg,
					"Editing hardbox down right cordinate for Seq %d, frame "
					"%d.  SHIFT to move fast. Control to move one"
					" pixel.  TAB for next edit option. ESC to return to "
					"sprite picker. S to save to dink.ini. X: %d Y: %d",
					sp_seq, sp_frame,
					k[seq[sp_seq].frame[sp_frame]].hardbox.right,
	...
	if (g_editorMode == EDITOR_MODE_SCREEN_HARDNESS) {
		...
		if (defaulthardness == curhardness)
			sprintf(str, "Current %03d (default %03d tile %03d)", curhardness,
					defaulthardness, sourcetile);
		else
			sprintf(str, "Current %03d (default %03d tile %03d) (alternate)",
					curhardness, defaulthardness, sourcetile);

		sprintf(msg,
				"Alternative Tile Hardness Selector: Press S to stamp this "
				"tiles hardness."
...

	{
		...
		for (x = 0; x < 256; x++) {
			if (input_getscancodestate((SDL_Scancode)x)) {
				sprintf(msg + strlen(msg), " (Scancode %i '%s')", x,
						//SDL_GetScancodeName(x),
						SDL_GetKeyName(
								SDL_GetKeyFromScancode((SDL_Scancode)x)));
			}
		}
	}
	...
	rcRect.bottom = 480;
	if (show_display) {
		//Yeolde: commented out in favour of ImGui
		...
		if (input_getcharstate(SDLK_i)) {
			...
			for (j = 1; j < 100; j++) {
				if (cur_ed_screen.sprite[j].active == /*true*/ 1) {
					...
					int sprite2 = add_sprite_dumb(cur_ed_screen.sprite[j].x,
												cur_ed_screen.sprite[j].y, 0,
												cur_ed_screen.sprite[j].seq,
												cur_ed_screen.sprite[j].frame,
												cur_ed_screen.sprite[j].size);
					...
					box_crap.left = box_crap.left +
							((box_crap.right - box_crap.left) / 2);
					...
					// GFX
					{
						...
						IOGFX_backbuffer->blitNoColorKey(
								GFX_k[seq[10].frame[5]].k, NULL, &dst);
					}
					...
					strcpy(crap6, "");
					if (cur_ed_screen.sprite[j].hard == 0)
						strcpy(crap6, "HARD");

					sprintf(crap5, "B: %d %s", cur_ed_screen.sprite[j].brain,
							crap6);

					if (cur_ed_screen.sprite[j].type == 0) {

						SaySmall(crap5, box_crap.left + 3, box_crap.top + 3,
								255, 255, 255);
					}
					...
					if (strlen(cur_ed_screen.sprite[j].script) > 1)
						SaySmall(cur_ed_screen.sprite[j].script,
								box_crap.left + 3, box_crap.top + 35, 255, 0,
								0);

					sprintf(crap6, "%d", j);

					SaySmall(crap6, box_crap.left + 20, box_crap.top - 15, 0,
							255, 0);
				}
			}
		}
	...
	if (g_editorMode == EDITOR_MODE_SPRITE_HARDNESS) {
		...
		if ((sp_mode == 1) | (sp_mode == 2)) {
			...
			// GFX
			{
				...
				dst.w = k[seq[sp_seq].frame[sp_frame]].hardbox.right -
						k[seq[sp_seq].frame[sp_frame]].hardbox.left;
				dst.h = k[seq[sp_seq].frame[sp_frame]].hardbox.bottom -
						k[seq[sp_seq].frame[sp_frame]].hardbox.top;
				IOGFX_backbuffer->fillRect(&dst, 41, 0, 49);
			}
		}
	...
	if (in_enabled) {
		...
		//check_joystick();
		if (input_getscancodestate(SDL_SCANCODE_RETURN) ||
			input_getscancodestate(SDL_SCANCODE_KP_ENTER)) {
			//exit text mode
			...
			if (in_command == 2) {
				if (in_string != NULL)
					...
					log_error("Error, in_char pointer not set, can't issue a "
							"value.");
			}
			...
			if (in_command == 1) {
				if (in_int != NULL) {
					...
					if ((old_command == INPUT_SCREEN_MIDI) ||
						(old_command == INPUT_SCREEN_TYPE))
						g_dmod.map.load();
	...
	editor_mode = g_editorMode;
	cur_sprite = last_sprite_added;
	...
	if (draw_screen_tiny > 0) {
		...
		log_debug("tiny is %d, X is %d and Y is %d", draw_screen_tiny, (draw_screen_tiny - 1) * 20 - (((draw_screen_tiny - 1) / 32) * 640), (((draw_screen_tiny - 1) / 32) - huh) * 20);
		shrink_screen_to_these_cords((draw_screen_tiny - 1) * 20 - (((draw_screen_tiny - 1) / 32) * 640), (((draw_screen_tiny - 1) / 32) - huh) * 20);
		copy_screen_big((draw_screen_tiny - 1) * 600 - ((((draw_screen_tiny - 1) / 32) * (600 * 32))), ((((draw_screen_tiny - 1) / 32) - huh) * 400));
	...
	//Main flip actually done here
	g_display->clear();
	//Draw the standard editor display to screen if drawgame is on, and the editor popout isn't active
	//With the editor popout active, render the boundary preview to the display
	if (debug_drawgame)
	g_display->flipStretch(IOGFX_backbuffer);
	else {
	//The rect is related to the current screen
	SDL_Rect r;
	//what we're drawing to
	SDL_Rect s = {0,0, g_display->w, g_display->w};
	r.x = (scr - 1) * 600 - ((((scr - 1) / 32) * (600 * 32)));
	r.y = ((scr -1) / 32) * 400;
	r.h = g_display->h + r.x;
	r.w = g_display->w + r.y;
	//log_debug("x: %d, y %d, h %d, w %d", r.x, r.y, r.h, r.w);
	//Blit the temporary buffer with the contents of our "minimap" buffer
	IOGFX_tmp1->blit(IOGFX_tmp3, &r, &s);
	//Make sure to render the backbuffer too
	//IOGFX_backbuffer->blitStretch(IOGFX_background, NULL, NULL);
	//Make the screen display with our repurposed tempbuffer
	//IOGFX_backbuffer->blit(IOGFX_background, NULL, NULL);
	g_display->flip(IOGFX_tmp1, &s, 0, 1);
	}
	/*
...
void AppFreeDinkedit::input(SDL_Event* ev) {
	...
	if (ev->type == SDL_TEXTINPUT && strlen(in_temp) < in_max &&
		isprint(ev->text.text[0]))
		sprintf(in_temp + strlen(in_temp), "%c", ev->text.text[0]);

	if (g_editorMode == EDITOR_MODE_SCREEN_SPRITES && !debug_input)
