...
int getpic(int sprite_no) {
	...
	if (spr[sprite_no].pseq >= MAX_SEQUENCES) {
		log_error("Sequence %d?  But max is %d!", spr[sprite_no].pseq,
				MAX_SEQUENCES);
		return 0;
	}
	...
	if (spr[h].pseq > 0) {
		...
		if (seq[spr[h].pseq].frame[1] == 0) {
			if (seq[spr[h].pseq].is_active)
				...
				log_error(
						"Error: sprite %d references non-existent sequence %d",
...
bool live_sprite_is_expired(int h, Uint32 thisTickCount) {
	return spr[h].kill_ttl > 0 &&
			(spr[h].kill_start + spr[h].kill_ttl < thisTickCount);
}
		...
		if (spr[h].seq > 0) {
			if (spr[h].frame < 1) {
				...
				if (spr[h].frame_delay != 0)
					...
					spr[h].delay = (thisTickCount +
									seq[spr[h].seq].delay[seq[spr[h].seq].len]);
			} else {
				// not new anim
				//is it time?
				if (thisTickCount > spr[h].delay) {
					...
					if (spr[h].seq > 0 &&
						seq[spr[h].seq].special[spr[h].frame] == 1)
						*p_isHitting = true;
				}
			}
		}
