 ...
 */
int load_screen_to(const char* path, const int num,
				struct editor_screen* screen) {
	...
	//log_debug("Trying to read %d bytes with offset of %d",lsize,holdme);
...
void save_screen(const char* path, const int num) {
	...
	if (num > 0) {
		...
		if (f == NULL) {
			//Yeolde: added this so that it creates the file if it doesn't exist. Plays havoc with accursed mapnuke mods.
			f = paths_dmodfile_fopen(path, "w+b");
			log_debug("Created new MAP.DAT");
			//return;
		}
		if (f == NULL) {
