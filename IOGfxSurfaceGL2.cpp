...
IOGfxSurfaceGL2::IOGfxSurfaceGL2(IOGfxDisplayGL2* display, GLuint texture,
								int w, int h, SDL_Color colorkey)
	: IOGfxSurface(w, h), display(display), texture(texture),
	colorkey(colorkey) {
	fbo = 0;
}
...
bool IOGfxSurfaceGL2::bindAsFramebuffer() {
	...
	if (fbo == 0) {
		...
		GLenum status;
		if ((status = gl->CheckFramebufferStatus(GL_FRAMEBUFFER)) !=
...
int IOGfxSurfaceGL2::fillRect(const SDL_Rect* dstrect, Uint8 r, Uint8 g,
							Uint8 b) {
	...
	display->setVertexAttrib(display->fillRect,
							display->fillRect->attributes["v_coord"],
							display->vboSpriteVertices, 4);

	glm::mat4 projection =
			glm::ortho(0.0f, 1.0f * display->w, 0.0f, 1.0f * display->h);
	glm::mat4 m_transform;
	m_transform = glm::translate(glm::mat4(1.0f),
								glm::vec3(dstrect->x, dstrect->y, 0.0)) *
			glm::scale(glm::mat4(1.0f), glm::vec3(dstrect->w, dstrect->h, 0.0));
	glm::mat4 mvp = projection * m_transform; // * view * model * anim;
	gl->UniformMatrix4fv(display->fillRect->uniforms["mvp"], 1, GL_FALSE,
						glm::value_ptr(mvp));
	gl->Uniform4f(display->fillRect->uniforms["color"], r / 255.0, g / 255.0,
				b / 255.0, 1.0);
...
int IOGfxSurfaceGL2::internalBlit(IOGfxSurface* src, const SDL_Rect* srcrect,
								SDL_Rect* dstrect, bool useColorKey) {
	if (src == NULL)
		return SDL_SetError(
				"IOGfxSurfaceGL2::blitStretch: passed a NULL surface");
	if (dstrect == NULL)
		return SDL_SetError(
				"IOGfxSurfaceGL2::blitStretch: passed a NULL dstrect");
	...
	GLuint vbo;
	if (srcrect == NULL ||
		(srcrect->x == 0 && srcrect->y == 0 && srcrect->w == src_surf->w &&
		srcrect->h == src_surf->h)) {
		...
		gl->BufferData(GL_ARRAY_BUFFER, sizeof(croppedSpriteTexcoords),
					croppedSpriteTexcoords, GL_STATIC_DRAW);
		vbo = display->vboCroppedSpriteTexcoords;
	}

	gl->UseProgram(display->blit->program);
	display->setVertexAttrib(display->blit,
							display->blit->attributes["v_coord"],
							display->vboSpriteVertices, 4);
	display->setVertexAttrib(display->blit,
							display->blit->attributes["v_texcoord"], vbo, 2);
	...
	m_transform = glm::translate(glm::mat4(1.0f),
			...
			glm::scale(glm::mat4(1.0f), glm::vec3(dstrect->w, dstrect->h, 0.0));
	glm::mat4 mvp = projection * m_transform; // * view * model * anim;
	gl->UniformMatrix4fv(display->blit->uniforms["mvp"], 1, GL_FALSE,
						glm::value_ptr(mvp));

	if (useColorKey && src_surf->colorkey.a == SDL_ALPHA_TRANSPARENT)
		gl->Uniform3f(display->blit->uniforms["colorkey"],
					src_surf->colorkey.r / 255.0,
					src_surf->colorkey.g / 255.0,
					src_surf->colorkey.b / 255.0);
	else
