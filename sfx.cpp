...
static void callback_samplerate(int chan, void* stream, int len, void* udata) {
	...
	case AUDIO_U8:
		...
		and v2 (measured between 0 and (1<<8)-1) */
				int subpos =
						data->pos & ((1 << 8) - 1); /* or: data->pos % 256 */
...
int CreateBufferFromWaveFile_RW(SDL_RWops* rwops, int rwfreesrc, int index) {
	...
	if (SDL_LoadWAV_RW(rwops, rwfreesrc, &wav_spec, &wav_buf, &wav_len) ==
		NULL) {
		log_error("Could not open sound file: %s", SDL_GetError());
		return 0;
	}
	log_info("frequency=%dHz\tformat=%s\tchannels=%d\tlength=%d bytes",
			wav_spec.freq, format2string(wav_spec.format), wav_spec.channels,
			wav_len);
...
SDL_audiocvt.c. */
	...
	/* Build AudioCVT */
	ret = SDL_BuildAudioCVT(&cvt, wav_spec.format, wav_spec.channels,
							wav_spec.freq, hw_format, hw_channels,
							wav_spec.freq);
...
format2string(cvt.dst_format), cvt.len_cvt);*/
	...
	int pos_end = ((wav_len / wav_bytesPerSample) / wav_spec.channels)
			<< 8; /* number of frames */
	int pos_lastframe =
			(((wav_len / wav_bytesPerSample) / wav_spec.channels) - 1) << 8;
...
played continuously when arrow keys are pressed */
	if (registered_sounds[sound].cvt.buf != NULL)
		SoundPlayEffectChannel(sound, registered_sounds[sound].orig_spec.freq,
							0, 0, 0, 0);
}
...
static int SoundPlayEffectChannel(int sound, int min, int plus, int sound3d,
	...
	if (registered_sounds[sound].cvt.buf == NULL) {
		...
		log_exit("SoundPlayEffectChannel: %d", 0);
		return 0;
	}
...
play one frame with hw_freq */
		...
		log_trace("Call Mix_QuickLoad_RAW", channel);
...
should be cleaned from a non-callback function). */
		...
		log_trace("Call Mix_PlayChannel: explicit_channel: %d", explicit_channel);
		channel = Mix_PlayChannel(explicit_channel, chunk, -1);
		log_debug("channel from Mix_PlayChannel: %d", channel);
		if (channel < 0) {
			log_error("Mix_PlayChannel: Error playing sound %d - %s", sound,
					Mix_GetError());
			log_exit("SoundPlayEffectChannel: %d", 0);
			return 0;
		}
		if (channel > NUM_CHANNELS) {
			log_error("Mix_PlayChannel returned channel (%d) that exceeds "
					"max available channels: %d", channel, NUM_CHANNELS);
			log_exit("SoundPlayEffectChannel: %d", 0);
			return 0;
		}
		log_trace("SoundPlayEffectChannel: Calling Mix_Pause for channel: %d", channel);
		Mix_Pause(channel);
		...
		struct callback_data* data =
				(struct callback_data*)calloc(1, sizeof(struct callback_data));
		data->pos = 0;
		data->shift = shift;
		data->sound = sound;
		data->channel = channel;

		log_trace("SoundPlayEffectChannel: Calling Mix_RegisterEffect for channel: %d", channel);
		Mix_RegisterEffect(channel, callback_samplerate,
						callback_samplerate_cleanup, data);
		Mix_ChannelFinished(CleanupChannel);
		log_trace("SoundPlayEffectChannel: Calling Mix_Resume for channel: %d", channel);
		Mix_Resume(channel);
	}
	...
	channel++;
	log_exit("SoundPlayEffectChannel: %d", channel);
	return channel;
}
...
int SoundStopEffect(int sound) {
	...
	if (sound >= MAX_SOUNDS) {
		log_error("Attempting to get stop sound %d (> MAX_SOUNDS=%d)", sound,
				MAX_SOUNDS);
		return 0;
	}
...
void sfx_init() {
	...
	//Yeolde: Change this to your full path to a soundfont (sf2/dls) to have it play MIDIs
	Mix_SetSoundFonts("/Users/tree/Acapella.sf2");
...
4096 is considered too big for SFX */
	...
	if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT,
					MIX_DEFAULT_CHANNELS, buf_samples) == -1) {
	...
	/* Test SDL_mixer capabilities */
	{
		...
		for (i = 0; i < total; i++) {
			...
			if (strcmp(Mix_GetMusicDecoder(i), "MP3") == 0)
				log_info("Audio music decoder: MP3 (MP3 is patented, prefer "
						"Ogg Vorbis!)");
			else
 ...
 */
static void CleanupChannel(int channel) {
	log_enter("CleanupChannel: channel: %d", channel);
...
free the Chunk when it's done playing. */
	...
	if (chunk == NULL) {
		log_fatal("Internal error: cannot free channel %d's chunk (where did "
				"it disappear?)",
				channel);
		exit(1);
	}
...
static int SetVolume(int channel, int dx_volume) {
	...
	return Mix_Volume(channel,
					MIX_MAX_VOLUME * pow(10, ((double)dx_volume / 100) / 20));
}
...
static int SetPan(int channel, int dx_panning) {
	...
	/* See doc/sound.txt for details */
	if (dx_panning > 0)
		return Mix_SetPanning(
				channel, 255 * pow(10, ((double)-dx_panning / 100) / 20), 255);
	else
		return Mix_SetPanning(channel, 255,
							255 * pow(10, ((double)dx_panning / 100) / 20));
}
...
int playsound(int sound, int min, int plus, int sound3d, int repeat) {
	...
	log_debug("sound played on channel: %d", channel);
	int soundbank = channel + 1;

	log_exit("playsound: %d", soundbank);
	return channel;
}
