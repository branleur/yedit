...
void ImageLoader::initBlitFormat(Uint32 format) {
	...
	ImageLoader::blitFormat =
			SDL_CreateRGBSurface(0, 1, 1, bpp, Rmask, Gmask, Bmask, Amask);
	if (format == SDL_PIXELFORMAT_INDEX8) {
		SDL_SetPaletteColors(ImageLoader::blitFormat->format->palette,
							GFX_ref_pal, 0, 256);
	}
}
...
SDL_Surface* ImageLoader::convertToPaletteFormat(SDL_Surface* image) {
	SDL_Surface* converted =
			SDL_CreateRGBSurface(0, image->w, image->h, 8, 0, 0, 0, 0);
	...
	SDL_SetPaletteColors(converted->format->palette,
						blitFormat->format->palette->colors, 0, 256);
	return converted;
}
...
SDL_Surface* ImageLoader::newWithBlitFormat(int w, int h) {
	...
	if (blitFormat->format->format == SDL_PIXELFORMAT_INDEX8) {
		...
		Uint32 Rmask = 0, Gmask = 0, Bmask = 0, Amask = 0;
		int bpp = 0;
		SDL_PixelFormatEnumToMasks(blitFormat->format->format, &bpp, &Rmask,
								&Gmask, &Bmask, &Amask);
