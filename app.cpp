...
//yeolde: for meson for now
#define PACKAGE "yeoldedinkedit"
#define BUILD_DATA_DIR "."
#define VERSION 1
#define PACKAGE_BUGREPORT "billy@cock.li"
#define PACKAGE_STRING "yeoldedinkedit"
#define PACKAGE_NAME "yeoldedinkedit"
#define LOCALEDIR "po"
...
static void android_set_LANG() {
	...
	// locale.toString()
	mid = env->GetMethodID(localeClass, "toString", "()Ljava/lang/String;");
...
== /storage/sdcard0/Android/data/org.freedink/files */
	...
	if (SDL_AndroidGetExternalStoragePath() == NULL)
		log_error("Could not get external storage path '%s': %s'",
				SDL_AndroidGetExternalStoragePath(), SDL_GetError());
	...
	if (chdir(SDL_AndroidGetExternalStoragePath()) < 0)
		log_error("Could not chdir to '%s': %s'",
				SDL_AndroidGetExternalStoragePath(), strerror(errno));
	...
	if (chdir(SDL_GetBasePath()) < 0)
		log_error("Could not chdir to '%s': %s'", SDL_GetBasePath(),
				strerror(errno));
#endif
}
...
void App::print_version() {
	...
	printf("FreeDink is free software, and you are welcome to redistribute "
		"it\n");
...
void App::print_help(int argc, char* argv[]) {
	...
	printf(_("  -r, --refdir <dir>    Specify base directory for "
			"dink/graphics, D-Mods, etc.\n"));
	printf("\n");
	printf(_("  -d, --debug           Explain what is being done\n"));
	printf(_("  -i, --noini           Do not attempt to write "
			"dinksmallwood.ini\n"));
	printf(_("  -j, --nojoy           Do not attempt to use joystick\n"));
	printf(_("  -s, --nosound         Do not play sound\n"));
	printf(_("  -t, --truecolor       Allow more colours (for recent D-Mod "
			"graphics)\n"));
	printf(_("  -w, --window          Use windowed mode instead of screen "
			"mode\n"));
...
bool App::check_arg(int argc, char* argv[]) {
	...
	optind = 0;
	//Yeolde: turn off 8-bit mode permanently
	truecolor = 1;
	...
	while ((c = getopt_long_only(argc, argv, short_options, long_options,
		...
		case 'S':
			...
			break;
		case ',':
			printf(_("Note: -nomovie is accepted for compatibility, but has no "
					"effect.\n"));
			break;
		default:
			return false;
...
void App::one_iter() {
	...
	SDL_Event* ev = &event;
	input_reset();
	//Yeolde: Changed this to capture events for imgui
