...
char* get_fontconfig_path(char* fontname) {
	...
	if (FcPatternGetString(fs->fonts[0], FC_FILE, 0, &strval) ==
				FcResultTypeMismatch ||
		strval == NULL) {
		log_error("get_fontconfig_path: cannot find font filename for %s",
				fontname);
		return NULL;
	}
...
int gfx_fonts_init() {
	...
	system_font = TTF_OpenFontRW(
			SDL_RWFromConstMem(vgasys_fon, sizeof(vgasys_fon)), 1, FONT_SIZE);
	if (system_font == NULL) {
		log_error("Failed to load built-in 'vgasys.fon' font: %s",
				TTF_GetError());
		return -1;
	}
...
static TTF_Font* load_default_font() {
	...
	if (rwops == NULL) {
		...
		if (rwops != NULL)
			log_info("Loaded LiberationSans-Regular.ttf from %s",
					paths_getpkgdatadir());
	}
#ifdef HAVE_FONTCONFIG
	if (rwops == NULL) {
		char* path = get_fontconfig_path("Arial:style=Regular");
		rwops = SDL_RWFromFile(path, "rb");
		if (rwops != NULL)
			log_info("Loaded Arial from "
					"fontconfig at %s",
					path);
		free(path);
	}
	...
	if (font_object == NULL) {
		log_error("Could not open font 'LiberationSans-Regular.ttf': %s",
				TTF_GetError());
		return NULL;
	}
...
void set_font_color(int no, int r, int g, int b) {
	if (no >= 1 && no <= 15 && r >= 0 && r <= 255 && g >= 0 && g <= 255 &&
		b >= 0 && b <= 255) {
		font_colors[no].r = r;
		font_colors[no].g = g;
		font_colors[no].b = b;
...
/*static*/ SDL_Surface* print_text(TTF_Font* font, char* str,
								SDL_Color /*&*/ color) {
	if (strlen(str) == 0)
		return NULL;
	...
	if (!truecolor) {
		...
		SDL_GetRGB(phys_index, ImageLoader::blitFormat->format, &(color.r),
				&(color.g), &(color.b));
		SDL_FreeFormat(fmt);
	}
 ...
 */
static int font_len(TTF_Font* font, char* str, int len) {
	int text_w;
	char* tmp;
 ...
 * (a.k.a. word-wrapping)
 */
static int process_text_for_wrapping(TTF_Font* font, char* str, int max_len) {
...
void print_text_cmds(std::vector<TextCommand>* cmds) {
	for (std::vector<TextCommand>::iterator it = cmds->begin();
		it != cmds->end(); ++it) {
		if (!truecolor)
...
IOGfxSurface* print_text_flatten_cmds(std::vector<TextCommand>* cmds) {
	...
	SDL_Surface* flat =
			ImageLoader::newWithBlitFormat(max_x - min_x, max_y - min_y);
	...
	do {
		...
		for (std::vector<TextCommand>::iterator it = cmds->begin();
			it != cmds->end(); ++it) {
			if (memcmp(&it->img->format->palette->colors[1], &cur,
					sizeof(SDL_Color)) == 0) {
				key++;
				found = true;
				break;
...
int print_text_wrap(char* str, rect* box,
	...
	std::vector<TextCommand> cmds;
	int res = print_text_wrap_getcmds(str, box, hcenter, calc_only, font_type,
									&cmds);
 ...
 */
void print_text_wrap_debug(const char* text, int x, int y) {
	...
	*pc = '\0';
	...
	while (!this_is_last_line) {
		...
		SDL_Surface* rendered_text =
				TTF_RenderUTF8_Shaded(system_font, pline, text_color, bgcolor);
		if (!truecolor)
