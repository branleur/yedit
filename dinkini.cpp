 ...
 */
void make_idata(enum idata_type type, int myseq, int myframe, int xoffset,
				int yoffset, rect crect) {
	int i;
	...
	log_error("Out of idata spots (max is %d), no more sprite corrections can "
			"be allowed.",
			nb_idata);
}
...
void program_idata(void) {
	...
	for (i = 1; i < nb_idata; i++) {
		...
		if (id[i].type == IDATA_SPRITE_INFO) {
			...
			rect_copy(&k[seq[id[i].seq].frame[id[i].frame]].hardbox,
					&id[i].hardbox);
...
void pre_figure_out(char* line, bool playmidi) {
	...
	// LOAD_SEQUENCE  path  seq  speed  offsetx offsety  hard.left hard.top hard.right hard.bottom
		...
		make_idata(IDATA_SPRITE_INFO, myseq, myframe, atol(ev[3]), atol(ev[4]),
				hardbox);
	}
	...
	// SET_FRAME_FRAME  seq frame  -1
		...
		make_idata(IDATA_FRAME_FRAME, myseq, myframe, new_seq, new_frame,
				hardbox);
	}
