...
//yeolde: my temporary surface for full map prev
IOGfxSurface* IOGFX_tmp3 = NULL;
...
int gfx_init(bool dinkgl, bool windowed, char* splash_path) {
	...
	if (dinkgl)
		...
		g_display =
				new IOGfxDisplaySW(GFX_RES_W, GFX_RES_H, truecolor,
								windowed ? SDL_WINDOW_RESIZABLE
											: SDL_WINDOW_FULLSCREEN_DESKTOP);
	...
	IOGFX_tmp1 = g_display->allocBuffer(GFX_RES_W * 3, GFX_RES_H * 3);
	IOGFX_tmp2 = g_display->allocBuffer(GFX_RES_W, GFX_RES_H);
	IOGFX_tmp3 = g_display->allocBuffer(GFX_RES_W * 32, GFX_RES_H * 24);
	...
	/* Display splash picture, as early as possible */
	{
		...
		FILE* in = paths_pkgdatafile_fopen(splash_path, "rb");
		if (in == NULL) {
...
void gfx_quit() {
	...
	if (IOGFX_backbuffer != NULL)
		delete IOGFX_backbuffer;
	if (IOGFX_background != NULL)
		delete IOGFX_background;
	if (IOGFX_tmp1 != NULL)
		delete IOGFX_tmp1;
	if (IOGFX_tmp2 != NULL)
		delete IOGFX_tmp2;
